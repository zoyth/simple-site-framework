...
export function generateThemeCSS(theme: ThemeConfigV1): string {
  return `
    /* Brand Colors */
...
export function generateDesignTokens(theme: ThemeConfigV1): string {
  // Border radius mapping
  const radiusMap: Record<ThemeConfigV1['design']['borderRadius'], string> = {
    sharp: '0',
    rounded: '0.375rem',
    pill: '9999px',
  };
  ...
  // Shadow mapping
  const shadowMap: Record<ThemeConfigV1['design']['shadows'], string> = {
    flat: 'none',
  ...
  // Spacing mapping
  const spacingMap: Record<ThemeConfigV1['design']['spacing'], { section: string; element: string }> = {
    compact: {
      section: '3rem',
      element: '1rem',
    },
